...
def get_interpolation_coefficients(c, degree, tolerance=TOLERANCE):
    z = []
    lambda_ = 1.0
...
def get_initial_causal_coefficient(c, z, tolerance=TOLERANCE):
    z1 = z
    zn = z ** (len(c) - 1)
    sum_ = c[0] + zn * c[-1]
    horizon = len(c)
...
def get_initial_anti_causal_coefficient(c, z, tolerance=TOLERANCE):
