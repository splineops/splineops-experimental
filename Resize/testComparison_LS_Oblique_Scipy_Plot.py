...
def main():
    ...
    method_oblique = "Oblique projection"
    ...
    for zoom_factor in zoom_factors:
        ...
        _, _, ls_snr, ls_mse = resize_and_compute_snr(
            input_image_normalized, method_ls, interpolation_type, zoom_factor
        )
        ...
        # Measure time and process images with oblique projection method
        _, _, oblique_snr, oblique_mse = resize_and_compute_snr(
            input_image_normalized, method_oblique, interpolation_type, zoom_factor
        )
        oblique_snrs.append(oblique_snr)
        oblique_mses.append(oblique_mse)

    # Plot SNR for all methods
    plt.figure(figsize=(12, 6))
    plt.plot(zoom_factors, ls_snrs, label=f'{method_ls} Method SNR', marker='o')
    plt.plot(zoom_factors, scipy_snrs, label='SciPy Method SNR', marker='x')
    plt.plot(zoom_factors, oblique_snrs, label=f'{method_oblique} Method SNR', marker='s')
    ...
    # Plot logarithmic MSE for all methods
    plt.figure(figsize=(12, 6))
    plt.plot(zoom_factors, np.log10(ls_mses), label=f'{method_ls} Method log(MSE)', marker='o')
    plt.plot(zoom_factors, np.log10(scipy_mses), label='SciPy Method log(MSE)', marker='x')
    plt.plot(zoom_factors, np.log10(oblique_mses), label=f'{method_oblique} Method log(MSE)', marker='s')
    plt.xlabel('Zoom Factor')
    plt.ylabel('log(MSE)')
